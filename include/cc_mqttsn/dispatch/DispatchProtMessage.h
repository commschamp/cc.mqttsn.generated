...
/// @file
/// @brief Contains dispatch to handling function(s) for Prot platform input messages.
...
/// @note Defined in cc_mqttsn/dispatch/DispatchProtMessage.h
template<typename TProtOptions, typename TId, typename TMsg, typename THandler>
auto dispatchProtMessage(
    TId id,
    TMsg& msg,
...
/// @brief Dispatch message object to its appropriate handling function.
/// @details Same as other dispatchProtMessage(), but receives extra @b idx parameter.
/// @tparam TProtOptions Protocol options struct used for the application,
///     like @ref cc_mqttsn::options::DefaultOptions.
/// @param[in] id Numeric message ID.
    ...
    THandler& handler) -> decltype(handler.handle(msg))
{
    ...
    return dispatchProtMessage<TProtOptions>(id, msg, handler);
}

/// @brief Dispatch message object to its appropriate handling function.
/// @details Same as other dispatchProtMessage(), but passing
...
auto dispatchProtMessageDefaultOptions(
    TId id,
    TMsg& msg,
    THandler& handler) -> decltype(handler.handle(msg))
{
    return dispatchProtMessage<cc_mqttsn::options::DefaultOptions>(id, msg, handler);
}

/// @brief Dispatch message object to its appropriate handling function.
/// @details Same as other dispatchProtMessageDefaultOptions(), 
///     but receives extra @b idx parameter.
/// @param[in] id Numeric message ID.
/// @param[in] idx Index of the message among messages with the same ID.
/// @param[in] msg Message object held by reference to its interface class.
/// @param[in] handler Reference to handling object.
/// @see dispatchProtMessageDefaultOptions()
/// @note Defined in cc_mqttsn/dispatch/DispatchProtMessage.h
template<typename TId, typename TMsg, typename THandler>
auto dispatchProtMessageDefaultOptions(
    TId id,
    ...
    THandler& handler) -> decltype(handler.handle(msg))
{
    return dispatchProtMessage<cc_mqttsn::options::DefaultOptions>(id, idx, msg, handler);
}
...
struct ProtMsgDispatcher
{
    ...
    /// @return What the @ref dispatchProtMessage() function returns.
    template <typename TId, typename TMsg, typename THandler>
    static auto dispatch(TId id, std::size_t idx, TMsg& msg, THandler& handler) ->
        decltype(cc_mqttsn::dispatch::dispatchProtMessage<TProtOptions>(id, idx, msg, handler))
    {
        return cc_mqttsn::dispatch::dispatchProtMessage<TProtOptions>(id, idx, msg, handler);
    }
    ...
    static auto dispatch(TId id, TMsg& msg, THandler& handler) ->
        decltype(cc_mqttsn::dispatch::dispatchProtMessage<TProtOptions>(id, msg, handler))
    {
        return cc_mqttsn::dispatch::dispatchProtMessage<TProtOptions>(id, msg, handler);
    }
...
/// @brief Message dispatcher class to be used with
///     @b comms::processAllWithDispatchViaDispatcher() function (or similar).
/// @details Same as ProtMsgDispatcher, but passing
///     @ref cc_mqttsn::options::DefaultOptions as template parameter.
/// @note Defined in cc_mqttsn/dispatch/DispatchProtMessage.h
using ProtMsgDispatcherDefaultOptions =
    ProtMsgDispatcher<>;
